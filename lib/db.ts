...
export async function userIdFromSession(token?:string){
  ...
  const db:any=getSupabaseAdmin();
  const {data,error}=await db.from('qz_sessions').select('user_id,expires_at').eq('token',token).gt('expires_at',now().toISOString()).maybeSingle();
...
export async function inventoryForStore(storeId:string){
  ...
  try {
    ...
    const existing=new Set((data??[]).map((x: InventoryDbRow)=>String(x.product_id)));
    ...
    const rows=(data??[]).map((x: InventoryDbRow)=>toInventory(x));
...
export async function createPaymentSession(userId:string,input:{storeId:string;mode:'pickup'|'delivery';items:{id:string;qty:number}[]}){
  ...
  try {
    ...
    for(const raw of input.items){
      ...
      const inv=invRows.find((x: InventoryRow)=>x.storeId===input.storeId&&x.productId===product.id);
...
export async function listOrders(userId:string):Promise<DbOrder[]>{
  ...
  return (data??[]).map((x: OrderRow)=>toOrder(x));
}
