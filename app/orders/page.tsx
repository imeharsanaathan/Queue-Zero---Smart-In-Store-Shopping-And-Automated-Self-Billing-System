    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:py-12">
      ...
      {orders.length===0 ? <div className="mt-8 rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center"><PackageCheck className="mx-auto h-10 w-10 text-slate-300"/><div className="mt-4 text-lg font-black">No orders yet</div><div className="mt-1 text-sm text-slate-500">Complete a payment and your order history will appear here.</div></div> : <div className="mt-8 space-y-4">
        {orders.map((order:DbOrder)=><article key={order.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-5 grid gap-3 border-t border-slate-100 pt-5 sm:grid-cols-2">{order.items.map((item:OrderItem)=>{const product=allProducts.find(p=>p.id===item.id);return <div key={`${order.id}-${item.id}`} className="flex min-w-0 items-center gap-3 rounded-2xl bg-slate-50 px-3 py-3 sm:px-4"><div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white ring-1 ring-slate-200">{product?.image?<img src={product.image} alt="" className="h-full w-full object-contain p-1.5"/>:<div className="text-[10px] font-black text-slate-300">No image</div>}</div><div className="min-w-0 flex-1"><div className="break-words text-sm font-black leading-5">{item.name}</div><div className="mt-1 text-xs font-semibold text-slate-500">Qty {item.qty}</div></div><span className="shrink-0 text-sm font-black">{money(item.price*item.qty)}</span></div>})}</div>
        </article>)}
      </div>}
    </div>
  </main>;
